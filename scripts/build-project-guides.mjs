...
${project.steps?.length ? `<section><h2>${esc(project.stepsTitle || (project.demo ? 'Try it step by step' : 'The intended user journey'))}</h2>${list(project.steps)}</section>` : ''}
...
${project.ending ? `<section class="ending-section"><p class="section-index">${esc(project.endingIndex || '06 / Postmortem')}</p><h2>${esc(project.endingTitle || 'What changed')}</h2>${paragraphs(project.ending)}</section>` : `<section><h2>Limits and unfinished work</h2>${paragraphs(project.limits)}${project.references?`<p>${project.references.map(([label,url])=>`<a href="${url}">${esc(label)}</a>`).join(' · ')}</p>`:''}</section>`}
