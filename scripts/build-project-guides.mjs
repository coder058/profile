...
<section><h2>What depends on what</h2>${table('Components, dependencies and responsibility', ['Component','Depends on','Responsibility'], project.dependencies, 'Project dependency table')}</section>
${project.records ? `<section><h2>${esc(project.recordsTitle || 'How the data was organised')}</h2>${paragraphs(project.recordsIntro || '')}${table(project.recordsCaption || 'Tables, files and keys', ['Record','Depends on','Responsibility'], project.records, 'Data organisation table')}</section>` : ''}
